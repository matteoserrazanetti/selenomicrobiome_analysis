...
give aboslute paths in all folder requirements
...
def main_function(arg_list):
  ...
  if opt['S']:
    ...
    marker_list=create_marker_list(opt['sp4_cfg'])
    sp_fam_list=create_sp_fam_list(opt['sp4_cfg'], marker_list)
...
def create_sp_fam_list(sp4_cfg, marker_list):
  sp_fam_list = []
...
def create_marker_list(sp4_cfg):
  marker_list = []
  with open(sp4_cfg,'r') as infile:
    for line in infile:
      if line[0]=='f':
        line=line.split()
        if line[0]=='families_set.bact_markers':
          l = line[2].split(',')
          for el in l:
            marker_list.append(el)
  return(marker_list)
...
def count_selenoprotein_families(output_folder, id, marker_list, sp_fam_list):
  with open(output_folder+'/sp4/'+id+'/'+id+'/sp4_results_'+id+'.gff', 'r') as infile:
    count=0
    sp_fam_dict_partial = {}
  ...
  with open(output_folder+'/sp4/'+id+'/'+id+'/sp4_family_count_'+id+'.txt','w') as outfile:
    outfile.write('  '+id+'\n')
  ...
  for dir, subdirs, files in os.walk(input_folder):
    for file in files:
      if file.split('.')[-1] in fasta_ext:
        arg_list.append([dir+'/'+file,opt,fasta_ext])
