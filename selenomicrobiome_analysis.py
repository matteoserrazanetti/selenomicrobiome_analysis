...
def_opt= {'i':'', 'o':'','db':'undefined_DB',
          ...
          'e':1e-5,  'n':16, 'p':'prokarya','gtf':True, 'gff':True,
...
marker_list = ['tigr04348', 'egtB', 'seld', 'yqeb', 'yqec', 'ybbb','ybbb_rev','tigr04348_new']
  ...
  for dir, subdirs, files in os.walk(input_folder):
    for file in files:
      ...
      if file.split('.')[-1] in fasta_ext:
        print(file)
        absolute_path_file = dir+'/'+file
        id=file.split('.')[0]
        if opt['S']:
          print('Starting selenoprofiles4 for '+id)
          sp_fam_list=create_fam_list(sp4_cfg, marker_list)
          selenoprofiles(absolute_path_file, output_folder, opt['p'], id)
        ...
        if opt['T']:
          ...
          secmarker(absolute_path_file, output_folder, cpu, id)
          secmarker_count(absolute_path_file, output_folder, id)
        if opt['I']:
          print('Starting infernal for '+id)
          infernal(absolute_path_file, output_folder, cpu, cm_file, id)
